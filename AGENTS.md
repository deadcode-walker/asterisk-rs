...
asterisk-rs (umbrella, feature-gated re-exports)
  ...
  |     event.rs      -- AmiEvent enum (13 variants + Unknown), implements core::Event
  ...
  +-- asterisk-rs-ari (HTTP + WebSocket, port 8088)
        ...
        event.rs      -- AriEvent enum (serde tagged on "type"), Channel/Bridge/Playback types
...
- AMI: `AmiEvent` enum with 13 typed variants + `Unknown { event_name, headers }`
- ARI: `AriEvent` enum with serde `#[serde(tag = "type")]` + `#[serde(other)] Unknown`
