...
mod tests {
    ...
    #[test]
    fn test_generate_pending_id_unique() {
        let ids: HashSet<String> = (0..100).map(|_| generate_pending_id("test")).collect();
        assert_eq!(ids.len(), 100, "all generated IDs should be unique");
    }

    #[test]
    fn test_generate_pending_id_prefix() {
        let id = generate_pending_id("channel");
        assert!(
            id.starts_with("channel-pending-"),
            "expected 'channel-pending-' prefix, got: {id}"
        );

        let id = generate_pending_id("bridge");
        assert!(
            id.starts_with("bridge-pending-"),
            "expected 'bridge-pending-' prefix, got: {id}"
        );

        let id = generate_pending_id("playback");
        assert!(
            id.starts_with("playback-pending-"),
            "expected 'playback-pending-' prefix, got: {id}"
        );
    }

    #[test]
    fn test_generate_pending_id_counter_increments() {
        ...
        assert!(n2 > n1, "counter should increment: {n1} vs {n2}");
    }

    #[test]
    fn test_shared_counter_across_prefixes() {
        // all pending types share the same counter, so IDs are globally unique
        let a = generate_pending_id("channel");
