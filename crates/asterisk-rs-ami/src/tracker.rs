...
mod tests {
    ...
    #[test]
    fn test_extract_unique_id_from_newchannel() {
        let event = AmiEvent::NewChannel {
        ...
        assert_eq!(extract_unique_id(&event), Some("1234.1"));
    }

    #[test]
    fn test_extract_unique_id_returns_none_for_fullybooted() {
        let event = AmiEvent::FullyBooted {
            status: "Fully Booted".into(),
        };
        assert_eq!(extract_unique_id(&event), None);
    }
    ...
    async fn test_tracker_processes_call_lifecycle() {
        let bus = asterisk_rs_core::event::EventBus::<AmiEvent>::new(64);
        ...
        assert!(call.duration >= std::time::Duration::ZERO);
