...
| A long task recovers from durable state | completed modernization evidence remains checked in and new complex work creates one active plan | fresh reviewer states objective, exact checkpoint, risk, and next slice from repository files | a later session resumes without relying on this conversation | none |
